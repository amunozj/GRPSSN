...
#strdate='20170531_instance0_corr'
#strdate='20170531_instance0_Daily'
#strdate='20170601_instance2_Daily'
#strdate='20170601_instance0_Daily'
#strdate='20170601_quimby_time_intervals'
#strdate='20170601_spoerer_time_intervals'
#strdate='20170603_quimby_time_intervals'
#strdate='20170603_spoerer_time_intervals'
#strdate='20170604_quimby_time_intervals'
#strdate='20170603_spoerer_time_intervals'
#strdate='20170607_weber_time_intervals'
#strdate='20170608_time_intervals_short'
#strdate='20170612_time_intervals_short'
strdate='20170911_time_intervals_short'
...
#different periods for same observers - from max to max instead of min to min

#QUIMBY
#observers=['quimby', 'quimby', 'quimby', 'quimby', 'quimby']
#tcal1=[1900,1900,1912,1902,1913]
#tcal2=[1921,1911,1921,1913,1921]
observers=['quimby', 'quimby', 'quimby', 'quimby', 'quimby', 'quimby', 'quimby', 'quimby']
tcal1=[1900,1900,1912,1902,1913,1902,1901,1900]
tcal2=[1921,1911,1921,1913,1921,1921,1921,1919]
#SPOERER
#observers=['spoerer','spoerer','spoerer','spoerer','spoerer']
#tcal1=[1865,1865,1880,1865,1878]
#tcal2=[1893,1880,1893,1877,1893]
#observers=['spoerer','spoerer','spoerer','spoerer','spoerer','spoerer','spoerer','spoerer']
#tcal1=[1865,1865,1880,1865,1878,1867,1867,1866]
#tcal2=[1893,1880,1893,1877,1893,1892,1893,1892]
observers=['spoerer','spoerer','spoerer']
tcal1=[1865,1865,1878]
tcal2=[1893,1877,1893]

#observers=['weber','weber','weber','weber','weber','weber','weber','weber']
#tcal1=[1859,1867,1867,1859,1863,1867,1860,1859]
#tcal2=[1883,1879,1883,1879,1883,1877,1883,1880]

#observers=['shea','shea','shea','shea','shea','shea','shea']
#tcal1=[1847,1856,1847,1847,1850,1847,1847]
#tcal2=[1866,1866,1856,1864,1866,1860,1865]



#observers=['winkler','winkler','winkler','winkler','winkler','winkler','winkler']
#tcal1=[1889,1900,1889,1902,1889,1895,1889]
#tcal2=[1910,1910,1902,1910,1905,1910,1900]
...
#	file='corr_d_'+obs+'.dat' # corr files
	...
	print 'FILE *', file, '*'
	nbo=str(i).strip()
	fileObscsv=open(dirOut+'Observer_'+obs+nbo+'_parameters_'+strdate+'.csv','w')
	filecheckcsv=open(dirOut+'Observer_'+obs+nbo+'_CheckFracmonthFile.csv','w')
	...
	tcal1s=str(tcal1[i]).strip()
	tcal2s=str(tcal2[i]).strip()
	...
	for j in range(0,len(monthd)):
		if ndmonth0[j] >= limitnd:
			print 'FRACMONTH', yeard[j],monthd[j],ndmonth[j],fracdmonth1[j],ndmonth[j]/ndmonth0[j],ngmonthd[j],ngmonthd0[j],ndmonth0[j],ngmonthd[j]/ndmonth0[j],ngmonthd0[j]/ndmonth0[j]
	...
	for j in range(0,len(fracdmonth0)):
		print 'WRITING LOOP',j, ndmonth0[j], limitnd,float(datemonth0[j].year)
		if ndmonth0[j] < limitnd :
	...
	savefig(dirOut+obs+nbo+'Fracmonth_'+strdate+'.png',dpi=72)
	#*********************************************************************
	#*********************************************************************
	...
	savefig(dirOut+obs+nbo+'_ADF_GN_'+strdate+'.png',dpi=72)
	#*********************************************************************
	...
	plt.ylabel('ADF / month')
	savefig(dirOut+obs+nbo+'_ADF_SNGN_'+strdate+'.png',dpi=72)
	#*********************************************************************
	#*********************************************************************
	...
	for j in range(0,len(ADF)-1):
		...
		histoplot=[] ; histoplot_wolf=[] ; values_ADF=[]
		...
		if NG > 0.:
			...
			for jj in range(0,NG):
				...
				values_ADF.append(fracdmonth0[titi[0][jj]])
		...
		if len(histoplot) > 1.:
			...
			plt.ylabel('Number')
			savefig(dirOut+obs+nbo+'_HISTO_GN_ADF_'+ADFS+'_'+strdate+'.png',dpi=72)
			#*********************************************************************
			#*********************************************************************
			...
			plt.ylabel('Number')
			savefig(dirOut+obs+nbo+'_HISTO_SN_ADF_'+ADFS+'_'+strdate+'.png',dpi=72)
			#*********************************************************************
	#import pdb; pdb.set_trace()
	...
	savefig(dirOut+obs+'Fracmonth_all_'+tcal1s+'_'+tcal2s+strdate+'.png',dpi=72)
	#*********************************************************************
...
#	figname2=figure(i+add,figsize=(10.0,8.0),dpi=90)
	...
	plt.plot(ADF[0:-1],cdf[0:-1], color=cmap((i+1.)/ float(nbobs)), linewidth=1, label=obs+':'+tcal1s+'_'+tcal2s) 
	plt.errorbar(ADF[0:-1],cdf[0:-1], yerr=errcdf[0:-1],color='black', fmt='o')
	plt.plot(ADF[0:-1],cdf[0:-1],'o', color=cmap((i+1.)/ float(nbobs)), linewidth=1, alpha=0.5,markersize=2) 
	#plt.plot(dateintersect,gn2_inters,'.', color='b', linewidth=1, alpha=0.25, label=obs2) 
	plt.axis([0.,0.95, 0.,3.5*cdf[-2]])
	legend(loc='lower right',prop={'size':11},labelspacing=0.2)
	...
	savefig(dirOut+obs+nbo+'_HISTO_ADF_'+strdate+'.png',dpi=72)
	#*********************************************************************
